...
import { Lead, LeadBase } from '../types.ts';
        ...
        <section className="space-y-6">
          ...
          <div className="space-y-4">
            {pendingLeads.length > 0 ? pendingLeads.map(lead => {
              const base = bases.find(b => b.id === lead.baseId);
              return (
                <div key={lead.id} className="glass overflow-hidden rounded-[2rem] border border-white/5 hover:border-indigo-500/30 transition-all flex flex-col sm:flex-row group">
                  {base?.image && (
                    <div className="w-full sm:w-32 h-32 sm:h-auto overflow-hidden bg-slate-800">
                      <img src={base.image} alt="Base" className="w-full h-full object-cover transition-transform group-hover:scale-110" />
                    </div>
                  )}
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex flex-col gap-0.5">
                        ...
                        <p className="text-slate-600 text-[10px] font-bold uppercase tracking-wider">
                          Base: {base?.name || 'Desconhecida'}
                        </p>
                      </div>
                    </div>
                    <button 
                      onClick={() => handleDispatch(lead)}
                      className="bg-indigo-600 hover:bg-indigo-500 text-white px-5 py-3 rounded-2xl flex items-center gap-2 font-bold transition-all transform hover:scale-[1.05] shadow-lg shadow-indigo-600/20 ml-4"
                    >
